...
public abstract class SyncModule {
    ...
    private final void bind(IBinder service) {
        ...
        if (service == null) {
            ...
            /*make sure this run in glass*/
                intent = new Intent(SERVICE);
            } else {
			/*make sure this run in mobile*/
                intent = new Intent();
                intent.setAction(mContext.getPackageName() + ".SYNC_SERVICE");
    ...
    public boolean hasLockedAddress() throws SyncException {
        ...
        String addr = getLockedAddress();
        return addr.equals("") ? false : true;
    }
