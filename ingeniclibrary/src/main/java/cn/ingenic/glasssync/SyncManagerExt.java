...
public class SyncManagerExt extends DefaultSyncManager implements
		TransportManager.OnRetriveCallback {
	...
	protected SyncManagerExt(Context context) {
		...
		if (tranMgr != null) {
			mTransportManager = tranMgr;
			mTransportManager.setRetriveCallback(this);
		} else {
	...
	public int send(String module, SyncData data) {
		Log.d(TAG, "send module:" + module + "syncData:" + data.getString("d"));
	...
	public void onRetrive(SyncSerializable serial) {
		...
		//支持Linux平台解绑
		if (!Enviroment.getDefault().isWatch() && module.equals("system_module")) {
			SyncData data = SyncSerializableTools.serial2Data(serial, true);
			data.setFlowDirrect(false);
			data.retain();
			boolean unbind = data.getBoolean("unbind", false);
			if (unbind) {
				Intent intent = new Intent();
				intent.setAction(RECEIVER_REQUST_UNBIND);
				mContext.sendBroadcast(intent);				
			} else {
				//手机主动解绑时,手机端会等待SystemModule数据返回将线程wait住，此时要进行notify
				Module m = getModule("SYSTEM");
				IRemoteBinder binder = m.getRemoteService("RemoteChannelManagerService");
				binder.onReply(2, null);
			}
		}
	...
	return mTransportManager.getPkgEncodeSize(type);
    }
