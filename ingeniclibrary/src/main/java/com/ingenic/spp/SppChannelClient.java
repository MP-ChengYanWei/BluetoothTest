...
class SppChannelClient extends SppChannel {
    ...
    private synchronized void internalConnect(BluetoothDevice device, UUID uuid, boolean secure) {
        if (D) Log.i(TAG, "internalconnect to: " + device);
        ...
        bindDevice = device;//将连接的设备传递给父类
        bindUUID = uuid;//将UUID传递给父类
    ...
    private synchronized void connected(BluetoothSocket socket, BluetoothDevice
        ...
        if (D) Log.i(TAG, "connected device addr: "+device.getAddress()
    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                if(D) Log.i(TAG, "start connect addr: "+mmDevice.getAddress()+" uuid: "+mmUuid);
                mmSocket.connect();
                if(D) Log.i(TAG, "finish connect");
            } catch (Exception e) {
                // Close the socket
                try {
                    Log.e(TAG, "close mmSocket e: "+e); 
                    mmSocket.close();
                } catch (Exception e2) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
	         ...
	         case MESSAGE_READ_TIMEOUT:
    		    Log.w(TAG,"spp client channel read timeout");
		    mListener.onRead(null, OnChannelListener.TIMEOUT_READ);
    		    break;
            }//end switch.
        }//end handleMessage.
    };//end mHandler.
