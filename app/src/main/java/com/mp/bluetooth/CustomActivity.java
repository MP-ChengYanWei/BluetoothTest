...
public class CustomActivity extends AppCompatActivity implements View.OnClickListener, RadioGroup.OnCheckedChangeListener {
...
//        int msgCycle = Integer.parseInt(cycle);
     ...
     * 发送数据
     */
    private void sendMsg(final int msgCount, final long msgCycle, final SendDataModel sendDataModel) {
        new Thread(new Runnable() {
            ...
            public void run() {
                for (int i = 0; i < msgCount; i++) {
                    ...
                    sendDataModel.getData().setMsgid(System.currentTimeMillis() + "");
                    Gson gson = new Gson();
                    String msg = gson.toJson(sendDataModel);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (action) {
                case BTConstants.APP_CONNECT_STATE_CHANGE_ACTION:
                    ...
                    if (!connect) {
                        Toast.makeText(CustomActivity.this, "连接已断开", Toast.LENGTH_SHORT).show();
                        getSupportActionBar().setTitle("连接断开了");
                    } else {
                        getSupportActionBar().setTitle("已连接");
                        Toast.makeText(CustomActivity.this, "重新连接", Toast.LENGTH_SHORT).show();
                    }
                    break;
